...
function init () {
  ...
  filters(eventListObj);
}

function filters (eventListObj) {
...
function renderEvents (activeDate) {
  ...
  function addFilterHandler () {
    function addFilterHandler () {
      ...
      filterWrapper.addEventListener('change', () => {renderEvents(activeDate)});
    }
    addFilterHandler();
  }
...
function renderCalendar () {
  function getCalendar () {
    ...
    for (let i = 1; i < activeDate.getDay(); i++) {
      table += "<td></td>";
    }
    ...
    while (activeDate.getMonth() === month) {
      ...
      if (activeDate.getDate() === day) {
        table += "<td class='table-active'>" + activeDate.getDate() + "</td>";
      } else {
        table += "<td>" + activeDate.getDate() + "</td>";
      }
      if (activeDate.getDay() % 7 === 0) {
        table += "</tr><tr>";
      }
    ...
    if (activeDate.getDay() !== 0) {
      for (let i = activeDate.getDay(); i <= 7; i++) {
        table += "<td></td>";
      }
    }
  ...
  function handlerDateChange (e) {
    ...
    const activeDate = new Date((new Date()).setDate(activeDay)); //Преобразование числа(активного дня) в полноценную дату.
